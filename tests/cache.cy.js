...
test('parallel GET requests are cached',
  ...
  ({ intercept, get, wait }) => {
    ...
    get('#root').then(([el]) => {
      el.innerHTML = `<div id="one" x-init="$ajax('/tests')"></div><div id="two" x-init="$ajax('/tests')"></div>`
    })
    wait('@response').then(() => {
...
test('staggered GET requests are NOT cached',
  ...
  ({ intercept, get, wait }) => {
    ...
    intercept('GET', '/tests', {
      statusCode: 200,
      body: `<div x-init><button id="one" x-on:click="$ajax('/tests')">ONE</button><button id="two" x-on:click="$ajax('/tests')">ONE</button></div>`
    }).as('response')
    get('#one').click()
    intercept('GET', '/tests', {
      statusCode: 200,
      body: `<div x-init><button id="one" x-on:click="$ajax('/tests')">TWO</button><button id="two" x-on:click="$ajax('/tests')">TWO</button></div>`
    }).as('response')
    get('#two').click()
    wait('@response').then(() => {
...
test('GET requests with different params are NOT cached',
  ...
  ({ intercept, get, wait }) => {
    ...
    // because this request fires immediately
    get('#root').then(([el]) => {
      el.innerHTML = `<div x-init="$ajax('/tests', { body: { 'NAME': 'ONE' } })" id="one"></div><div x-init="$ajax('/tests', { body: { 'NAME': 'TWO' } })" id="two"></div>`
    })
    wait('@response').then(() => {
      get('#one').should('have.text', 'ONE')
      get('#two').should('have.text', 'TWO')
