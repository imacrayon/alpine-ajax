...
function Ajax(Alpine) {
  ...
  Alpine.magic('ajax', (el) => {
    return async (action, options = {}) => {
      ...
      return request(control, action, method, body)
    }
  })

  Alpine.magic('requests', () => {
    return (id) => RequestStore.has(id) ? RequestStore.get(id) : []
  })
...
async function handleLinks(event) {
  ...
  try {
    return await request(control, action)
  } catch (error) {
...
async function handleForms(event) {
  ...
  try {
    return await withSubmitter(submitter, () => {
      return request(control, action, method, body, enctype)
    })
  } catch (error) {
    if (error.name === 'RenderError') {
      console.warn(error.message)
...
async function request(control, action = '', method = 'GET', body = null, enctype = 'application/x-www-form-urlencoded') {
  ...
  let request = {
    ...
    headers: Object.assign({
      ...
      'X-Alpine-Target': targets.map(target => target.id).join(' ')
  ...
  let pending
  if (method === 'GET' && RequestCache.has(request.action)) {
  ...
  let response = (await Promise.allSettled(RequestQueue)).pop().value
  RequestQueue.shift()
