...
function targets(root, trigger = null, sync = false) {
  ...
  if (ids.length === 0) {
    let description = (_b = ((_a = root.outerHTML.match(/<[^>]+>/)) != null ? _a : [])[0]) != null ? _b : "[Element]";
  ...
  if (sync) {
    document.querySelectorAll("[x-sync]").forEach((el) => {
      var _a2, _b2;
      if (!el.id) {
        let description = (_b2 = ((_a2 = el.outerHTML.match(/<[^>]+>/)) != null ? _a2 : [])[0]) != null ? _b2 : "[x-sync]";
        throw Error(`${description} is missing an ID to target.`);
      }
...
function navigateRequest(link) {
  ...
  return {
    ...
    referrer: (_a = link.closest("[data-source]")) == null ? void 0 : _a.dataset.source,
    body: null
  };
}
...
function src_default(Alpine) {
  var _a;
  setRenderer((_a = Alpine.morph) != null ? _a : (from) => {
